...
void leNome(char *nome){
    ...
    do{
        flag = 0;
        printf("Digite o nome do paciênte: ");
        gets(nome);
	if(strlen(nome) < 2){
	   flag = 1;
	}
...
*/
bool validaSoNumero(char *valor){
	int x;
	x = 0;
...
void CadastraCliente(cliente *P, int Idade, char CPF [], char nome [])
{
	int opcao;
	int caracteres;
    ...
    do{
        printf("Digite os dados do paciênte \n\n");
        leNome(nome);
        fflush(stdin);
        leCpf(cpf);
        fflush(stdin);
